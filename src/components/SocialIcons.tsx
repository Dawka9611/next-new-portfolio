...
const SocialIcons = () => {
    ...
    return (
        <IconsStyle className="relative flex justify-center box-border items-center">
            ...
            </div>
            {
                iconsData.map((el, ind) => (
                    // ${toggle ? `icon${ind + 1}` : ""}
                    toggle && <li color={el.color} key={ind} className={`absolute list-none hover:scale-200 `}
                        style={{
                            transform: `rotate(${ind * (360 / iconsData.length)}deg) translateY(-100px)`, transitionDelay: `0.05s * ${ind}`
                        }}>
                        <a href={el.url} target="_blank" style={{ color: el.color, transform: `rotate(${ind * (360 / -iconsData.length)}deg)`, width: "40px", height: "40px" }} className="anchor relative flex text-2xl justify-center items-center bg-white rounded-full hover:scale-150">
